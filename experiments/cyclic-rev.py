...
def ulsegx_z3(y):
    return Iff( lsegx(y), IteBool(y == x, True, lsegx(next(y))) )
...
def uclistx_z3(y):
    return Iff( clistx(y), And(y != nil, lsegx(next(y))) )
...
pfp_dict['clistx'] = '''
(and (not (= {primary_arg} nil)) (lsegx (next {primary_arg})))'''
pfp_dict['clistny'] = '''
(and (not (= {primary_arg} nil)) (lsegny (next {primary_arg})))'''

# Recall recursive predicates are always unary
fcts_z3['recpreds-loc_1_int_bool'] = [lsegy, lsegx, lsegny, clisty, clistx, clistny]

############# Section 5
# Program, VC, and Instantiation

def vc(z, w):
    return Implies( lsegy(z), Implies( clistx(z), lsegx(w) ) )
