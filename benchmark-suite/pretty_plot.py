...
def pretty_plot(x, y, x_name='FOSSIL[option]', y_name='FOSSIL', log=True, diagonal=True,
                x_leg='lower right', y_leg='center right', plotdir='./plots/'):
    """
    ...
    # Generate masks
    timeout_x = np.array([xi==np.max(data) for xi in data[0]])
    timeout_y = np.array([yi==np.max(data) for yi in data[1]])
    ...
    # Plot the timeouts
    x_t = np.exp(1.05*np.log(np.max(data[0])))*np.ones(len(np.where(timeout_x)[0]))
    ax_scatter.scatter(x_t, data[1,np.where(timeout_x)],
                       color='green', marker='>', s=60)
    y_t = np.exp(1.05*np.log(np.max(data[1])))*np.ones(len(np.where(timeout_y)[0]))
