...
valid_lemmas = [Not(lseg_y(yp))]
invalid_lemmas = []

cex_models = []
config_params = {'mode': 'random', 'num_true_models': 10}
...
# lemma = Implies(lseg_y(x), Implies(next(y) == yp, lsegkeys_yp(x) == SetAdd(lsegkeys_y(x), key(y))))
