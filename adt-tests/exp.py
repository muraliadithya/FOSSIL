...
exp = RecFunction('exp', fgsort, fgsort, fgsort)
qexp = RecFunction('qexp', fgsort, fgsort, fgsort, fgsort)
AddRecDefinition(plus, (x, y), If(x == zero, y, succ(plus(pred(x), y))))
AddRecDefinition(mult, (x, y), If(x == zero, zero, plus(mult(pred(x), y), y)))
AddRecDefinition(exp, (x, y), If(y == zero, succ(zero), mult(exp(x, pred(y)), x)))
AddRecDefinition(qexp, (x, y, z), If(y == zero, z, qexp(x, pred(y), mult(z, x))))
...
AddAxiom(x, mult(x, succ(zero)) == x)
AddAxiom((x, y, z), mult(exp(x, y), z) == qexp(x, y, z))

thm_to_prove = Implies(nat(x), Implies(nat(y), exp(x, y) == qexp(x, y, succ(zero))))
...
config_params['goal_instantiation_terms'] = {x, y, pred(y), succ(zero)}
