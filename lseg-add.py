...
def ulsegy_p_z3(x):
    return Iff( lsegy_p(x), IteBool(x == yp, True, lsegy_p(next_p(x))) )
...
# def uhlsegy_p_z3(x):
#     emptyset_intsort = getSortEmptySet(SetIntSort)
#     return hlsegy_p(x) == IteSet(x == yp, emptyset_intsort, SetAdd(hlsegy_p(next_p(x)),x) )
...
def pgm(x, y, yp):
    ...
    return And( lsegy(x), list(y), next(y) == yp )

def vc(x, y, yp):
    return Implies( pgm(x, y, yp), And( list_p(yp), lsegy_p(x) ) )

deref = [x]
const = [nil, y, yp]
# modified_set = {y, yp} # Note that this is a python set with the elements themselves
